...
### 2026-08-24
- **버그**: 2026-08-12~23 12일 연속으로 3월 포스트(3/11~3/24)가 순서 그대로 재발행됨. 주제 풀 142개가 8/11에 소진되면서 관점(angle) 재사용 모드로 진입했으나, 파일명 슬러그와 H1 제목이 원본 주제 그대로여서 본문만 다르고 제목·URL은 100% 동일한 글이 생성됨
- **수정**:
  - 슬러그와 제목에 관점을 반영 (`HTTP 캐싱 전략 — 성능 튜닝과 벤치마킹`). 기존 12건도 리네임하고 H1 갱신
  - `posts/` 디렉토리를 이력의 진실의 원천으로 변경. 이력 JSON이 사라지거나 손상돼도 파일명에서 주제·관점을 복원하므로 2026-07-07 유형의 사고가 재발하지 않음
  - 재사용 시 "가장 오래된 1개"가 아니라 오래된 후보 묶음에서 선택 — 과거 발행 순서를 그대로 되감는 문제 해소. 관점도 사용 빈도가 낮은 것부터 배분
  - 주제 풀 142개 → 203개 확장 (관점 8종 조합 시 약 4.4년 분량)
  - `scripts/generate.py` 334줄 단일 파일을 `topics`/`slugs`/`history`/`picker`/`generate` 5개 모듈로 분리하고 테스트 64개 추가 (커버리지 94%). 매일 발행 전 테스트를 먼저 통과해야 포스팅이 생성됨
...
| 언어 | Python 3.11 |
| 주제 중복 방지 | `posts/` 디렉토리 스캔 + LRU/관점 로테이션 (`.topic-history.json`은 기록용) |
...
│   └── YYYY-MM-DD-title.md
├── scripts/
│   ├── generate.py              # 진입점: 조립 + Claude API 호출
│   ├── topics.py                # 주제 풀(203개) / 관점 풀(8종)
│   ├── slugs.py                 # 제목·슬러그 생성 규칙
│   ├── history.py               # posts/ 기반 발행 이력 복원
│   └── picker.py                # 주제·관점 선택 로직
├── tests/                       # pytest 테스트 (64개)
└── .topic-history.json          # 발행 이력 기록 (posts/에서 재생성 가능)
```
